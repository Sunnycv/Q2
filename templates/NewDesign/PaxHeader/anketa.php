...
if($_POST['step3']!=''){
	$frm=$_SESSION['anketa'];
	if($frm['surname']!=''){$text.="Фамилия:".$frm['surname']."<br>";}
	if($frm['name']!=''){$text.="Имя:".$frm['name']."<br>";}
	if($frm['patronymic']!=''){$text.="Отчество:".$frm['patronymic']."<br>";}
	mail("sadukovigor@mail.ru", "Анкета", $text);
	echo $text;
}
?>
	<hr>
	...
	<div class='sogl'>
		...
		<p style="margin: 4px 0;">Я оставляю за собой право отозвать свое согласие посредством составления соответствующего письменного документа, который может быть направлен мной в адрес ООО МСК "УДОБНО" по почте заказным письмом с уведомлением о вручении</p>
	</div>
	<?}?>
	<form method='POST'>
		<?if($step==1){?>
		...
		<p>Все документы представляются в копиях. Указанные документы необходимо предоставить в ООО МСК "УДОБНО" при очном визите.</p>
		<fieldset>
